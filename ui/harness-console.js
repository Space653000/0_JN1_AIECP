...
const readNotificationIds=new Set(); // presentation-only; never written to Control Plane
...
function render(){
 ...
 const githubView=window.AECPDashboard.githubView(focusTask,events);
 h+='<section class="hc-card" id="hcGitHub"><div class="section-title"><h2>'+esc(tr('dashboard.github','GitHub view'))+'</h2></div><div class="hc-detail-grid">'
   +Object.entries({branch:githubView.branch,base:githubView.base,commit:githubView.commit,pr:githubView.pr,ciStatus:githubView.ciStatus,
     workflow:githubView.workflow,artifacts:githubView.artifacts,release:githubView.release,lastEvent:githubView.lastEvent})
     .map(([key,value])=>'<div>'+esc(tr('dashboard.github.'+key,key))+': '+esc(value)+'</div>').join('')+'</div></section>';
 ...
 h+='<section class="hc-card" id="hcNotifications"><div class="section-title"><h2>'+esc(tr('dashboard.notifications','Notifications'))+'</h2><span class="count-badge">'+notifications.length+'</span></div><div class="hc-events">'
   +(notifications.map(n=>'<div class="hc-event hc-notification" aria-label="'+esc(tr('dashboard.notification.'+n.category,n.category))+' '+esc(n.type)+'"><time>'+esc(fmt(n.at))+'</time><b>'+(n.category==='CRITICAL'||n.category==='ACTION REQUIRED'?'⚠ ':'')+esc(tr('dashboard.notification.'+n.category,n.category))+'</b><span>'+esc(n.type)+' · '+esc(n.taskId)+'</span><span>'+(n.approvalTarget?'<a href="#hcApprovals">'+esc(tr('dashboard.openApprovals','Open approvals'))+'</a> ':'')+'<button type="button" class="secondary-button" data-read-notification="'+esc(n.id)+'" aria-pressed="'+String(n.read)+'">'+esc(n.read?tr('dashboard.read','Read'):tr('dashboard.markRead','Mark read'))+'</button></span></div>').join('')||'<div class="empty-list">UNKNOWN · '+esc(tr('dashboard.noNotifications','No verified notifications'))+'</div>')+'</div></section>';
 ...
 document.querySelectorAll('[data-read-notification]').forEach(b=>b.onclick=()=>{readNotificationIds.add(b.dataset.readNotification);render();});
 ...
 document.querySelectorAll('[data-device-revoke]').forEach(b=>b.onclick=async()=>{if(!confirmText('Revoke this paired device?'))return;await window.aecp.revokeRemoteDevice(b.dataset.deviceRevoke);pairing=null;await refresh();});
 document.querySelector('#hcEmergency')?.addEventListener('click',async()=>{if(!confirmText('STOP ALL will cancel every active mission. Continue?'))return;for(const r of runs.filter(x=>['QUEUED','RUNNING','PAUSED','HUMAN_REQUIRED'].includes(x.state)))await window.aecp.cancelMission(r.id);await refresh();});
 ...
 document.querySelectorAll('[data-cancel-task]').forEach(b=>b.onclick=async()=>{if(!confirmText('Cancel only this task? Other workers and tasks will keep running.'))return;await window.aecp.cancelTask(b.dataset.run,b.dataset.task);await refresh();});
 document.querySelectorAll('[data-merge]').forEach(b=>b.onclick=async()=>{if(!confirmText('Merge this governed PR into main?'))return;await window.aecp.approveDelivery({runId:b.dataset.run,taskId:b.dataset.task,by:'human'});await refresh();});document.querySelectorAll('[data-reject]').forEach(b=>b.onclick=async()=>{await window.aecp.rejectMissionAction(b.dataset.reject);await refresh();});
}
