...
**介面語言（擁有者偏好）：** 所有介面預設顯示繁體中文（zh-TW），英文是可以切換的附加功能（右上角語言按鈕，選擇會記住）。新增任何介面文字都要同時提供中文翻譯（`ui/i18n.js` 的 `DICTIONARIES` 或 `PHRASES`／`PATTERNS`）；`tests/ui-default-language.test.cjs` 會檢查 `index.html` 的文字是否都有翻譯。
