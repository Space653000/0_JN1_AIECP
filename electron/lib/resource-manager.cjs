...
// Drops userinfo (user:password@ or token@) from http(s) remotes and redacts token-looking text; host and path stay.
function safeRemote(remote){return redactText(String(remote||'').replace(/^(https?:\/\/)[^\s/@]+@/i,'$1'));}
...
class ResourceManager{
 ...
 async scan(root){const base=path.resolve(root),repos=[],seen=new Set();const add=async p=>{const r=await gitRoot(p);if(!r)return;const rp=path.resolve(r),key=rp.toLowerCase();if(seen.has(key))return;seen.add(key);let remote='';try{remote=safeRemote(await run(['remote','get-url','origin'],rp))}catch{}let branch='';try{branch=await run(['branch','--show-current'],rp)}catch{}repos.push({id:'repo-'+crypto.createHash('sha1').update(key).digest('hex').slice(0,12),path:rp,branch:branch||'(detached)',remote,updatedAt:new Date().toISOString()})};await add(base);let entries=[];try{entries=await fs.readdir(base,{withFileTypes:true})}catch{return repos}for(const e of entries){if(!e.isDirectory()||e.name.startsWith('.'))continue;await add(path.join(base,e.name));if(repos.length>=32)break}this.state.workspaces[base]={root:base,repositories:repos,updatedAt:new Date().toISOString()};for(const r of repos)this.state.repositories[r.id]=r;await this.persist();return repos}
