...
contextBridge.exposeInMainWorld('aecp', Object.freeze({
  ...
  getAgentSettings: () => call('agents:settings:get'),
  setAgentSettings: (agentId, patch = {}) => call('agents:settings:set', { agentId, ...patch }),
  sayHiAgent: (agentId, model) => call('agents:say-hi', model ? { agentId, model } : { agentId }),
