...
2. **選範圍**：先讀 `.ai/WORK_ORDERS/` 編號最小且狀態 OPEN 的施工單（依編號順序；完成一張標 `DONE-PENDING-REVIEW` 後若仍有 OPEN 施工單，直接接續，最後一次回報），範圍與禁止事項以施工單為準；完成後在該單「Codex 回報」小節填寫並將狀態改為 `DONE-PENDING-REVIEW`（由 Claude Code 驗收後改 `CLOSED`）。無 OPEN 施工單時，才從 `STATUS.md` 選一個可由倉庫施工解決的未完成項目，列出對應需求、影響檔案與驗收證據。藍圖的新決策或驗收規則交由 Claude Code／使用者決定；Codex 依既有決策實作。
